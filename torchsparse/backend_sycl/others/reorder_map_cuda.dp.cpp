...
void reorder_out_in_map_kernel(
...
at::Tensor reorder_out_in_map_cuda(
  ...
  dpct::get_in_order_queue().parallel_for(
      sycl::nd_range<3>(
          sycl::range<3>(1, 1, (M + cta_M - 1) / cta_M * kernel_volume) *
              sycl::range<3>(1, 1, cta_M),
          sycl::range<3>(1, 1, cta_M)),
      exp_props, [=](sycl::nd_item<3> item_ct1) {
        reorder_out_in_map_kernel(out_in_map, reorder_loc, M, kernel_volume,
                                  split_mask_len, reorder_out_in_map);
      });
