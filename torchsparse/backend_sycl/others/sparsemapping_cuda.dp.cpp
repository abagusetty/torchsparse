...
#include <cstdio>
#include <vector>
#include "../hashmap/hashmap_cuda.dp.hpp"
#include <cmath>
...
inline type_int transform_coords(int *in_coords,
                                                    int *coords_min,
                                                    int *coords_max) {
  type_int cur = 0;
  int sizes[NDim];
...
template <typename type_int>  // int32_t or int64_t
inline void inverse_transform_coords(type_int *in_coords,
                                                         int *coords_min,
                                                         int *coords_max,
                                                         int *out_coords) {
...
void inverse_transform_coords_and_insert_kernel(
...
void downsample_grid_kmap_stage1_specialized_fast(
...
template <typename type_hashtable_device_view, typename type_int>  // int32_t or int64_t
void downsample_hashmap_kmap_stage3(type_hashtable_device_view table,
...
template <typename type_hashtable_device_view, typename type_int>  // int32_t or int64_t
void subm_hashmap_kmap_stage1(type_hashtable_device_view table,
...
template <typename type_hashtable_device_view, typename type_int>  // int32_t or int64_t
void subm_hashmap_kmap_stage2_odd_kernel(type_hashtable_device_view table,
                                      int n_points, int kernel_volume,
                                      int *in_coords, int *coords_min,
...
template <typename type_hashtable_device_view, typename type_int>  // int32_t or int64_t
void subm_hashmap_kmap_stage2_even_kernel(type_hashtable_device_view table,
                                                      int n_points, int kernel_volume,
                                                      int *in_coords, int *coords_min,
                                                      int *coords_max, int *kernel_sizes,
                                                      int *out_in_map) {
...
void get_masks_from_kmap_kernel(int n_points, int n_points_out,
...
std::vector<at::Tensor> build_kernel_map_subm_hashmap_int32(
  ...
  q_ct1.submit([&](sycl::handler &cgh) {
    auto table_get_device_view_ct0 = table.get_device_view();

    cgh.parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(1, 1, (int)ceil((double)n_points / 256)) *
                sycl::range<3>(1, 1, 256),
            sycl::range<3>(1, 1, 256)),
        exp_props, [=](sycl::nd_item<3> item_ct1) {
          subm_hashmap_kmap_stage1<hashtable32::device_view, int32_t>(
              table_get_device_view_ct0, n_points, kernel_volume, in_coords,
  ...
  if (kernel_volume % 2 != 0){
    ...
    q_ct1.submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for(
          sycl::nd_range<3>(
              sycl::range<3>(
                  1, 1,
                  (int)ceil((double)n_points * (kernel_volume / 2) / 256)) *
                  sycl::range<3>(1, 1, 256),
              sycl::range<3>(1, 1, 256)),
          exp_props, [=](sycl::nd_item<3> item_ct1) {
            subm_hashmap_kmap_stage2_odd_kernel<hashtable32::device_view,
                                                int32_t>(
                table_get_device_view_ct0, n_points, kernel_volume, in_coords,
                coords_min, coords_max, kernel_sizes, out_in_map);
          });
    }); // only support odd kernel shapes
  }
  else {
    ...
    q_ct1.submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for(
          sycl::nd_range<3>(
              sycl::range<3>(
                  1, 1, (int)ceil((double)n_points * (kernel_volume) / 256)) *
                  sycl::range<3>(1, 1, 256),
              sycl::range<3>(1, 1, 256)),
          exp_props, [=](sycl::nd_item<3> item_ct1) {
            subm_hashmap_kmap_stage2_even_kernel<hashtable32::device_view,
                                                 int32_t>(
                table_get_device_view_ct0, n_points, kernel_volume, in_coords,
                coords_min, coords_max, kernel_sizes, out_in_map);
          });
    }); // only support even kernel shapes
  }
    ...
    q_ct1.submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for(
          sycl::nd_range<3>(
              ...
              sycl::range<3>(1, 1, 256)),
          exp_props, [=](sycl::nd_item<3> item_ct1) {
            subm_hashmap_kmap_stage2_odd_kernel<hashtable::device_view,
                                                int64_t>(
                table_get_device_view_ct0, n_points, kernel_volume, in_coords,
                coords_min, coords_max, kernel_sizes, out_in_map);
          });
    ...
    q_ct1.submit([&](sycl::handler &cgh) {
      ...
      cgh.parallel_for(
          sycl::nd_range<3>(
              ...
              sycl::range<3>(1, 1, 256)),
          exp_props, [=](sycl::nd_item<3> item_ct1) {
            subm_hashmap_kmap_stage2_even_kernel<hashtable::device_view,
                                                 int64_t>(
                table_get_device_view_ct0, n_points, kernel_volume, in_coords,
                coords_min, coords_max, kernel_sizes, out_in_map);
          });
...
std::vector<at::Tensor> build_kernel_map_downsample_hashmap_int32(
  ...
  */
  ...
  q_ct1.submit([&](sycl::handler &cgh) {
    auto table_get_device_view_ct0 = table.get_device_view();
    auto final_out_coords_data_ptr_int_ct5 = final_out_coords.data_ptr<int>();

    cgh.parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(1, 1, (int)ceil((double)n_out_points_scalar / 256)) *
                sycl::range<3>(1, 1, 256),
            sycl::range<3>(1, 1, 256)),
        exp_props, [=](sycl::nd_item<3> item_ct1) {
          inverse_transform_coords_and_insert_kernel(
              table_get_device_view_ct0, n_out_points_scalar, out_coords,
              coords_min, coords_max, final_out_coords_data_ptr_int_ct5);
        });
  });
  ...
  q_ct1.submit([&](sycl::handler &cgh) {
    ...
    cgh.parallel_for(
        sycl::nd_range<3>(
            ...
            sycl::range<3>(1, 1, 256)),
        exp_props, [=](sycl::nd_item<3> item_ct1) {
          downsample_hashmap_kmap_stage3(table_get_device_view_ct0, n_points,
                                         n_out_points_scalar, kernel_volume,
                                         out_kmap, out_in_map);
        });
  });
...
std::vector<at::Tensor> build_mask_from_kmap(int n_points, int n_out_points,
  ...
  dpct::get_in_order_queue().parallel_for(
      sycl::nd_range<3>(
          sycl::range<3>(1, 1, ceil((double)max_kmap_size / 256)) *
              sycl::range<3>(1, 1, 256),
          sycl::range<3>(1, 1, 256)),
      exp_props, [=](sycl::nd_item<3> item_ct1) {
        get_masks_from_kmap_kernel(n_points, n_out_points, kernel_volume, kmap,
                                   kmap_sizes, cum_kmap_sizes, input_mask,
                                   output_mask);
      });
