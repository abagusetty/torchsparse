...
void convert_out_in_map_kernel(const int* out_in_map, int* out_in_map_t, int n, int kernel_volume){
...
void derive_bit_mask_from_out_in_map_kernel(int* out_in_map, int* bitmask, int valid_n, int n, int kernel_volume, int split_mask_num){
  auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
...
void convert_transposed_out_in_map(const at::Tensor out_in_map,
  ...
  dpct::get_in_order_queue().submit([&](sycl::handler &cgh) {
    const int *out_in_map_data_ptr_int_ct0 = out_in_map.data_ptr<int>();
    auto out_in_map_t_data_ptr_int_ct1 = out_in_map_t.data_ptr<int>();
    int out_in_map_size_ct2 = out_in_map.size(0);
    int out_in_map_size_ct3 = out_in_map.size(1);

    cgh.parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(
                1, 1, (out_in_map.size(0) * out_in_map.size(1) + 255) / 256) *
                sycl::range<3>(1, 1, 256),
            sycl::range<3>(1, 1, 256)),
        exp_props, [=](sycl::nd_item<3> item_ct1) {
          convert_out_in_map_kernel(out_in_map_data_ptr_int_ct0,
                                    out_in_map_t_data_ptr_int_ct1,
                                    out_in_map_size_ct2, out_in_map_size_ct3);
        });
  });
}
...
at::Tensor derive_bitmask_from_out_in_map(const at::Tensor out_in_map, const int split_mask_num, int valid_n) {
  ...
  auto exp_props = sycl::ext::oneapi::experimental::properties{
      sycl::ext::oneapi::experimental::use_root_sync};
  dpct::get_in_order_queue().submit([&](sycl::handler &cgh) {
    auto out_in_map_data_ptr_int_ct0 = out_in_map.data_ptr<int>();
    auto bitmask_data_ptr_int_ct1 = bitmask.data_ptr<int>();
    int out_in_map_size_ct3 = out_in_map.size(0);
    int out_in_map_size_ct4 = out_in_map.size(1);

    cgh.parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(1, 1,
                           (split_mask_num * out_in_map.size(0) + 255) / 256) *
                sycl::range<3>(1, 1, 256),
            sycl::range<3>(1, 1, 256)),
        exp_props, [=](sycl::nd_item<3> item_ct1) {
          derive_bit_mask_from_out_in_map_kernel(
              out_in_map_data_ptr_int_ct0, bitmask_data_ptr_int_ct1, valid_n,
              out_in_map_size_ct3, out_in_map_size_ct4, split_mask_num);
        });
  });
  return bitmask;
}
