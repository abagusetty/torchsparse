...
#define CONVERT_HALF2(pointer) (reinterpret_cast<sycl::half2 *>(&(pointer))[0])
#define CONVERT_INT4(pointer) (reinterpret_cast<sycl::int4 *>(&(pointer))[0])

template <typename scalar_t>
void gather_kernel(const int n_k, const int n_in, const int c,
...
template <typename scalar_t>
void scatter_kernel(const int n_in, const int n_out, const int c,
                               scalar_t *in_feat, scalar_t *out_feat,
                               const int *kmap, const bool transpose) {
  auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
  int index = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
  ...
  if (isfloat) {
    ...
    sycl::half2 cur_out_feat = CONVERT_HALF2(out_feat[out_pos * c + (j << 1)]);
    cur_out_feat = sycl::ext::intel::math::hadd2(
        cur_out_feat, CONVERT_HALF2(in_feat[i * c + (j << 1)]));
...
// fused gather
template <typename scalar_t>
void gather_all_kernel_pad_sep_with_mask(
...
// fused scatter
template <typename scalar_t>
void scatter_all_kernel_pad_sep(
  ...
  for (int k = 0; k < kernel_volume; k++) {
    ...
    if (isfloat) {
      dpct::atomic_fetch_add<sycl::access::address_space::generic_space>(
          &CONVERT_FLOAT(out_feat[out_pos * c + j]),
...
void scatter_all_kernel_pad_sep_with_mask_half(
    const int n, const int c, const int kernel_volume, sycl::half *in_feat,
    sycl::half *out_feat, const int *kmap, const int *kmap_sizes,
    const int *cum_kmap_sizes, const int *cum_buffer_sizes,
    const int *input_mask, const int *output_mask, const bool transpose,
    const bool precompute_mid) {
  auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
  ...
  sycl::half2 tmps[4];
  sycl::int4 tmps_int4[1];
  for (int k = 0; k < 4; k++) tmps[k].x() = tmps[k].y() =
...
#pragma unroll
      for (int l = 0; l < 4; l++) {
        tmps[l] = sycl::ext::intel::math::hadd2(
            tmps[l], *(reinterpret_cast<sycl::half2 *>(tmps_int4) + l));
      }
    }
  } else {
    for (int k = 0; k < kernel_volume; k++) {
...
void scatter_all_kernel_pad_sep_with_mask_float(
...
at::Tensor conv_forward_gather_scatter_cuda_latest(
  ...
  if (is_half) {
    ...
    q_ct1.parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(
                1, 1, ceil((double)(n_out_feats * n_out_channels) / 2048)) *
                sycl::range<3>(1, 1, 256),
            sycl::range<3>(1, 1, 256)),
        exp_props, [=](sycl::nd_item<3> item_ct1) {
          scatter_all_kernel_pad_sep_with_mask_half(
              n_out_feats, n_out_channels, kernel_volume,
              reinterpret_cast<sycl::half *>(out_buffer.data_ptr<at::Half>()),
    ...
    q_ct1.parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(
                1, 1, ceil((double)(n_out_feats * n_out_channels) / 1024)) *
                sycl::range<3>(1, 1, 256),
            sycl::range<3>(1, 1, 256)),
        exp_props, [=](sycl::nd_item<3> item_ct1) {
